...
void Game::run()
{
	...
	while (m_rendering_engine->run()
		...
		updateCamera(dtime);
		updateSound(dtime);
		interact::processPlayerInteraction(dtime, client, camera, m_game_ui.get(), hud, soundmaker,
		                                   itemdef_manager, input, runData, m_repeat_place_time, m_repeat_dig_time,
		                                   crack_animation_length, &m_game_formspec);
